...
%--------------------------------------------------------------------------
                        %% -- dydt_Ohara.m -- %%
% Description: solve differential equations from the 2011 O'Hara Model 

% Inputs:
% --> t - [double] time interval  
% --> statevar - [double array] model state variables   
% --> Id  - [double] stimulus 
% --> p - [struct] main model parameters 
% --> c - [struct] model parameters that are varied when creating population

% Outputs: 
% --> deriv - [double array] solved differentials   
% -------------------------------------------------------------------------
%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% jss=hss;
jss = 1.0/(1+exp(((V)+82.90)/6.086)); % write the equation again to avoid masking of hss alternation
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
thsp=(3.0*(1.0/(0.009794*exp(-(V+17.95)/28.05)+0.3343*exp((V+5.730)/56.66))));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
dhsp=(hssp-hsp)/thsp;
hp=Ahf*hf+Ahs*hsp;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% jpss = jss ; % write the equation again to avoid masking of jss alternation
jpss = 1.0/(1+exp(((V)+82.90)/6.086)); 
% tjp=1.46*tj; % write the equation again to avoid masking of tj alternation
tjp=(1.46*(2.038+1.0/(0.02136*exp(-(V+100.6)/8.281)+0.3052*exp((V+0.9941)/38.45))));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
djp=(jpss-jp)/tjp;
...
thL=200.0; % originally thL = 200.0
...
thLp=(3.0*200.0); % HF remodeling should be kept constant
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
dhLp=(hLssp-hLp)/thLp;
...
tap = (1.0515/(1.0/(1.2089*(1.0+exp(-(V-18.4099)/29.3814)))+3.5/(1.0+exp((V+100.0)/29.3814)))) ;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
dap=(assp-ap)/tap;
...
issp = 1.0/(1.0+exp(((V)+43.94)/5.711)); 
tiFp=(dti_develop*dti_recover*(4.562+1/(0.3933*exp((-(V+100.0))/100.0)+0.08004*exp((V+50.0)/16.59)))*delta_epi);
tiSp=(dti_develop*dti_recover*(23.62+1/(0.001416*exp((-(V+96.52))/59.05)+1.780e-8*exp((V+114.1)/8.079)))*delta_epi);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
diFp=(issp-iFp)/tiFp;
diSp=(issp-iSp)/tiSp;
ip=AiF*iFp+AiS*iSp;
...
fcass=1.0/(1.0+exp(((V)+19.58)/3.696));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
jcass = 1.0/(1.0+exp(((V)+19.58)/3.696)) ;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
tjca=75.0;
djca=(jcass-jca)/tjca;
...
fpss = 1.0/(1.0+exp(((V)+19.58)/3.696)) ;
tffp=(2.5*(7.0+1.0/(0.0045*exp(-(V+20.0)/10.0)+0.0045*exp((V+20.0)/10.0))));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
dffp=(fpss-ffp)/tffp;
fp=Aff*ffp+Afs*fs;
...
tfcafp=(2.5*(7.0+1.0/(0.04*exp(-(V-4.0)/7.0)+0.04*exp((V-4.0)/7.0))));
fcapss = 1.0/(1.0+exp(((V)+19.58)/3.696)) ;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
xs2ss=1.0/(1.0+exp((-((V)+11.60))/8.932));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
bt=4.75;
a_rel=0.5*bt;
%Jrel_inf=a_rel*(-ICaL)/(1.0+(1.5*p.K_HF_RyRCa/Cajsr)^8.0);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Jleak = p.K_HF_leak*c.G.Leak_total*0.0039375*Cansr/15.0;
Jleak = c.G.Leak_total*0.0039375*Cansr/15.0;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Jup= p.K_HF_SERCA*c.G.SERCA_total*((1.0-fJupp)*Jupnp+fJupp*Jupp) - Jleak;
...
Bcai=1.0/(1.0+p.cmdnmax*kmcmdn/(kmcmdn+Cai)^2.0+trpnmax*kmtrpn/(kmtrpn+Cai)^2.0);
