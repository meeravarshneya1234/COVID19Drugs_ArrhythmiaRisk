%% Figure 1B: Concentration-dependent effects of COVID-19 drugs on 
%% ventricular action potentials.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%"Investigational treatments for COVID-19 may increase ventricular
% arrhythmia risk through drug interactions" 

% By: Varshneya & Irurzun-Arana et al. 

% For questions, please contact Dr.Eric A Sobie -> eric.sobie@mssm.edu 
% or put in a pull request or open an issue on the github repository:
% https://github.com/meeravarshneya1234/COVID19Drugs_ArrhythmiaRisk.git. 

%--- Note:
% Results displayed in manuscript were run using MATLAB 2019b on a 64bit
% Intel Processor. For exact replication of figures it is best to use these
% settings.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%-- Initial Settings 
settings.PCL = 1000 ;  % Interval bewteen stimuli,[ms]
settings.stim_delay = 20 ; % Time the first stimulus, [ms]
settings.stim_dur = 2 ; % Stimulus duration
settings.stim_amp = 32; % Stimulus amplitude
settings.steady_state = true; % Start with each scenario's BL model steady state values. 
settings.variations = 1; % run one cell
settings.numbertokeep = 1; % determine how many beats to keep. 1 = last beat, 2 = last two beats
settings.gender ='M'; % male - M female -F
settings.phys_state = 'healthy'; % healthy or HF

%-- Run baseline model without drug block 
pert = settings_blockcurrents; 
settings.nBeats = 1; % number of beats to run 
datatable = runSim(settings,pert); % run simulation 
tbase = datatable.times;
Vbase = datatable.states(:,1);
APD_base = find_APD90(tbase,Vbase);

%-- Simulate each drug 
Drugs = {'Azithromycin','Chloroquine','Ritonavir','Lopinavir'};
D = 10; % study cell under 10*Cmax drug block 
settings.nBeats = 100;
for ii = 1:length(Drugs)

    % Run Drug Block Simulation   
    Cmax = drugfuncs.get_drug_data(Drugs{ii});
    Test_Concentration = D*Cmax;
    pert = settings_blockcurrents; % initialize channel block settings
    pert = drugfuncs.calculate_drugblock(pert,Drugs{ii},Test_Concentration); % calculate drug channel block 
    datatable = runSim(settings,pert);% run simulation 
    t = datatable.times;
    V = datatable.states(:,1);
    APD(ii) = find_APD90(t,V);
    
    % Plot results 
    figure
    plot(tbase,Vbase,'k--','linewidth',2) % plot baseline  
    hold on
    plot(t,V,'linewidth',2) % plot drug block  
    hold on
    title([Drugs{ii} ': ' num2str(APD(ii) - APD_base)])
    xlim([0 500])
    ylim([-100 50])
    
end